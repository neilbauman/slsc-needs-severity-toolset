#!/usr/bin/env python3
"""
Import Sri Lanka population and poverty data from HDX.
...
# Sri Lanka datasets on HDX
LKA_POPULATION_DATASET = {
    "name": "Sri Lanka - Subnational Population Statistics",
    "dataset_id": "e982aec3-478e-4910-ae5c-072ae2cf6933",
    "description": "Population by subnational administrative units"
}

LKA_POVERTY_DATASETS = [
    {
        "name": "Sri Lanka Multidimensional Poverty Index",
        "dataset_id": "b91a3e02-8d07-401b-8ae4-331794650be8",
        "description": "Multidimensional Poverty Index for Sri Lanka"
    },
    {
        "name": "Sri Lanka - Poverty",
        "dataset_id": "ff9a54fa-05d6-4d41-be49-20c2334de8d8",
        "description": "Poverty indicators for Sri Lanka"
    }
]
...
def download_and_process_file(resource_url: str, output_dir: Path, file_type: str = "csv") -> pd.DataFrame:
    ...
    file_path = output_dir / f"lka_{file_type}{file_ext}"
...
def import_population(supabase, country_id: str, output_dir: Path):
    ...
    print("\n" + "=" * 80)
    print("IMPORTING SRI LANKA POPULATION DATA")
    print("=" * 80)
    
    dataset_info = get_hdx_dataset_info(LKA_POPULATION_DATASET["dataset_id"])
    if not dataset_info:
    ...
    dataset_name = f"Sri Lanka Population - {admin_level}"
    dataset_description = f"Total population by {admin_level} administrative units for Sri Lanka. Source: HDX ({dataset_info.get('title', 'Unknown dataset')})"
...
def import_poverty(supabase, country_id: str, output_dir: Path):
    ...
    print("\n" + "=" * 80)
    print("IMPORTING SRI LANKA POVERTY DATA")
    print("=" * 80)
    ...
    for dataset_config in LKA_POVERTY_DATASETS:
    ...
    # Create dataset metadata
    dataset_name = f"Sri Lanka Poverty Rate - {admin_level}"
    dataset_description = f"Poverty rate by {admin_level} administrative units for Sri Lanka. Source: HDX ({dataset_info.get('title', 'Unknown dataset')})"
...
def create_poverty_placeholder(supabase, country_id: str):
    ...
    # Use national average poverty rate for Sri Lanka (~25% based on World Bank)
    ...
    # Create dataset metadata
    dataset_name = f"Sri Lanka Poverty Rate (Placeholder) - {admin_level}"
    dataset_description = f"Placeholder poverty rate dataset for Sri Lanka at {admin_level} level. " \
...
def main():
    print("=" * 80)
    print("Sri Lanka Population and Poverty Data Import from HDX")
    print("=" * 80)
    ...
    country_resp = supabase.table("countries").select("id, iso_code, name").eq("iso_code", "LKA").single().execute()
    ...
    output_dir = Path(__file__).parent.parent / "data" / "sri_lanka_data"
