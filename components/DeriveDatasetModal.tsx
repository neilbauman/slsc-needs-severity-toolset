...
export default function DeriveDatasetModal({
  ...
  const handlePreview = async () => {
    ...
    setPreview(data.filter((r: any) => r.admin_pcode));
  };

  const handleSave = async () => {
    setLoading(true);
    const { error } = await supabase.rpc('materialize_derived_dataset_v3', {
      base_a: baseA,
      base_b: baseB,
      method,
      target_admin_level: targetLevel,
      new_name: `Derived ${method.toUpperCase()} ${Date.now()}`,
      new_description: 'Auto-generated derived dataset',
    });
    setLoading(false);
    if (error) return alert(error.message);
    onOpenChange(false);
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-lg p-6 w-[850px] max-h-[90vh] overflow-y-auto">
        <h2 className="text-xl font-semibold mb-4">Derive New Dataset</h2>
        ...
        {preview.length > 0 && (
          <>
            ...
            <div className="flex justify-end">
              ...
              >
                {loading ? 'Saving…' : 'Save Derived Dataset'}
              </button>
            </div>
          </>
        )}
