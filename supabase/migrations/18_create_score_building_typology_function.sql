...
BEGIN
  FOR r IN (
    ...
    FROM pg_proc
    WHERE proname = 'score_building_typology'
...
-- Create the function with the correct parameter order to match frontend call
-- Error message shows Supabase expects: in_category_scores, in_dataset_id, in_instance_id, in_limit_to_affected, in_method, in_threshold
-- Since frontend always passes all parameters, we make them all required (no defaults) to match exact order
CREATE FUNCTION public.score_building_typology(
  in_category_scores JSONB,
  in_dataset_id UUID,
  in_instance_id UUID,
  in_limit_to_affected BOOLEAN,
  in_method TEXT,
...
BEGIN
  ...
  -- Get instance admin_scope and country_id for data isolation
  SELECT admin_scope, country_id INTO v_instance_admin_scope, v_country_id
  FROM instances
  ...
  -- If limiting to affected areas, get affected ADM3 codes (with country isolation)
  IF in_limit_to_affected AND v_instance_admin_scope IS NOT NULL AND array_length(v_instance_admin_scope, 1) > 0 THEN
    ...
    WHERE UPPER(TRIM(admin_level)) = 'ADM3'
      ...
      AND (v_country_id IS NULL OR country_id = v_country_id);
...
-- Grant execute permission (with correct parameter order: JSONB, UUID, UUID, BOOLEAN, TEXT, NUMERIC)
GRANT EXECUTE ON FUNCTION public.score_building_typology(JSONB, UUID, UUID, BOOLEAN, TEXT, NUMERIC) TO authenticated;
GRANT EXECUTE ON FUNCTION public.score_building_typology(JSONB, UUID, UUID, BOOLEAN, TEXT, NUMERIC) TO anon;
