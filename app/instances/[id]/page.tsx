...
export default function InstanceDashboard() {
  ...
  const [framework, setFramework] = useState<Row[]>([]);
  const [finals, setFinals] = useState<Row[]>([]);
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      const [{ data: iData }, { data: fwData }, { data: fnData }, { data: admData }] = await Promise.all([
      ...
      setInst(iData as Instance);
      setFramework((fwData ?? []) as Row[]);
      setFinals((fnData ?? []) as Row[]);
      setAdm1((admData ?? []) as Boundary[]);
      setLoading(false);
    };
    load();
  }, [id, supabase]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 print-safe">
        ...
        <div className="card p-3 lg:col-span-2">
          ...
          {affectedFC.features.length === 0 && (
            <div className="text-xs text-gray-500 mt-2">
              No polygons to show. Define affected area or ensure{' '}
        ...
        <div className="space-y-3">
          <div className="card p-3">
            ...
            <div className="text-xs text-gray-500 mb-3">Quick read of current scoring</div>
